...
See the chapter on Outputs. This is a brief summary. The output are shown with the following simple fit:
...
```{r quick-augment}
head(broom::tidy(fit, type="fitted.ytT"))
```
...
```{r quick-autoplot}
ggplot2::autoplot(fit, plot.type="observations")
```
