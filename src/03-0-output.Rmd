...
You can also get all the Kalman filter and smoother estimates for $\xx$ from `MARSSkf()`. See `?MARSSkf`.
...
To get the fitted values, the estimated $\yy$, using all the data:
```{r quick-ytT}
head(fitted(fit, type="ytT"))
```
To get the fitted values, the estimated $\yy$, using the data up to time $t-1$:
```{r quick-ytt1}
